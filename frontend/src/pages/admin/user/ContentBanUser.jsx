...
import { getContentBanUser, patchUnBanUser } from "../../../services/apiUser";

function ContentBanUser() {
  ...
  const defaultColumns = [
    ...
    setColumn({ title: "사유", key: "banReason" }),
    setColumn({ title: "처리일", key: "createdAt" }),
    setColumn({ title: "처리자", key: "createdUser" }),
    setColumn({ title: "밴해제일", key: "unBanAt" }),
    setColumn({ title: "밴취소자", key: "unBanUser" }),
  ];
  ...
  async function handleSearch() {
    try {
      const { contents, totalCount, comboPerPage } = await getContentBanUser(
        userId,
        currentPage,
        perPage
      );
  ...
  async function handleUnBan() {
    if (selectedRowKeys.length === 0) {
      showMessage("밴해제할 항목을 선택해주세요!", "warning");
      return;
    }
    ...
    try {
      ...
      selectedRowKeys.forEach((key) => {
        for (let index = 0; index < dataSource.length; index++) {
          ...
          fetchData.push({ banId: row.banId });
      ...
      const { message } = await patchUnBanUser(fetchData, userId);

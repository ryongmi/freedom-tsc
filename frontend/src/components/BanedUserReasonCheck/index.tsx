...
interface DataType {
  ...
  url: string; // 게시물 URL
  banedReason: string; // 사유
  createdAt: string; // 처리일
  createdUser: string; // 처리자
  unBanedAt: string; // 벤 해제일
  unBanedUser: string; // 벤 취소자
}
...
for (let i = 1; i <= 100; i++) {
  data.push({
    ...
    createdUser: `권력자`,
    unBanedAt: '2023년 10월 10일',
    unBanedUser: '권력자',
  });
}

export default function BanedUserReasonCheck() {
